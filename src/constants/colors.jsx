export const colors = {
	background: "#111119",
	backgroundScreen: "#0b0b12",
	primary: "#b96eff",
	inactive: "rgba(240,238,255,0.38)",
	border: "rgba(255,255,255,0.07)",
	surface: "#1a1a28",
	text: "#f0eeff",
	sub: "rgba(255,255,255,0.5)",
	placeHolder: "rgba(255,255,255,0.3)",
	backgroundBtn: "rgba(185,110,255,0.15)",
	btnBorder: "rgba(185,110,255,0.3)",
	toggle: "rgba(240,238,255,0.42)",
	active: "#4ade80",
	pending: "#fbbf24",
	paused: "#f87171",
	avatarBg: "rgba(185,110,255,0.15)",
	avatarT: "#2dd4bf",
	avatarTB: "#60a5fa",
};

export const colorALight = {
	bga: "rgba(185,110,255,0.15)",
	bgb: "rgba(45,212,191,0.15)",
	bgc: "rgba(251,191,36,0.15)",
	bgd: "rgba(248,113,113,0.15)",
	bge: "rgba(96,165,250,0.15)",
};

export const colorEdit = {
	bgEdit: "rgba(96,165,250,0.15)",
	bgEditBorder:"rgba(96,165,250,0.3)"
}

export const colorDelete = {
	bgDelete: "rgba(248,113,113,0.15)",
	bgDeleteBorder:"rgba(248,113,113,0.3)"
}

export const errorModal = {
	bg:"rgba(0,0,0,0.6)",
	iconWrap:"rgba(248,113,113,0.1)",
	btnBg:"rgba(248,113,113,0.15)",
	btnBorder: "rgba(248,113,113,0.3)"
}